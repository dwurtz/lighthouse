...
- **task** — something {user_first_name} committed to doing themselves ("email Jane back", "book the flight"). Satisfied when a recent event shows {user_first_name} doing it.
- **waiting** — something another person owes {user_first_name} ("**Joe** — send builder contact"). Satisfied when the committed person delivers the promised outcome, directly or indirectly.
...
- **Direct completion** — the task was "email Jane back" and an event shows {user_first_name} sent the email.
...
- **Canonical task example.** Open item (task): `send Joe the revised deck`. Recent event: sent email from {user_first_name} to Joe with subject "Revised deck" and an attachment. This is **satisfied** — {user_first_name} is the actor, the action happened.
...
1. **Direct satisfaction** — the committed person does the thing themselves. Joe promised "send quote" → Joe emails the quote.

2. **Indirect satisfaction** — the promised outcome lands, even via someone else. Patterns:
   - **Forwarded info** — Joe promised "send Jane's number", Jane herself emails {user_first_name} instead. Joe's commitment is fulfilled; {user_first_name} got what was owed.
   ...
   - **Promised event happened** — Joe promised "Jane will reach out about the project", and Jane does. The event simply happening IS the satisfaction.
   - **Answer provided by the named source** — the committed person said "Jane will get back to you with the price", and Jane sends the price.

**Canonical waiting example.** Open item (waiting): `**Joe Smith** — builder contact for detached garage`. Recent event: a new contact writes to {user_first_name}: "Joe Smith referred me — I build detached garages in the area." This is **satisfied**. Joe delivered the builder contact; the new contact is the contact. The committed person (Joe) shows up in the event as the referrer, which is decisive evidence.

A waiting item is **not satisfied** when:
- No event touches the commitment.
- **Partial progress isn't satisfaction.** "Joe said he'll get back to me next week" isn't Joe fulfilling; it's a reschedule.
- **A different person does the thing without any tie to the committed person.** Random builder emails out of the blue unrelated to Joe → not satisfied (even if the topic matches).
