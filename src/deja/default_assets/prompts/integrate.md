...
9. **Identify the signed-in account before asserting ownership.** Any authenticated page in the browser belongs to whichever identity is active in that tab. Before writing, check: is there an avatar, initial, email, workspace name, tenant/org label, or URL hint (`authuser=N`, `/u/N/`, subdomain, `?workspace=`) telling you which account owns this view? Attribute the signal to that account.
