"""FastAPI backend for the Déjà notch app.
...
from deja.config import DEJA_HOME, REFLECT_MODEL
from deja.prompts import load as load_prompt

app = FastAPI(title="deja", version="0.2.0")

# First-launch setup wizard endpoints
from deja.setup_api import router as setup_router
...
async def post_chat(body: dict):
    ...
    WIKI_DIR, and logs each mutation to log.md and deja.log. The
...
# ~/.deja/audio/. Click stop → POST /api/mic/stop → web server sends
...
async def _mic_stop_inner(reason: str = "manual") -> dict:
    ...
    import logging
    mic_log = logging.getLogger("deja.mic")
...
# ~/.deja/meeting_audio/<session-id>/. Python handles transcription
...
async def meeting_start(body: dict) -> dict:
    ...
    import logging
    logging.getLogger("deja.meeting").info(
...
async def meeting_stop(body: dict = None) -> dict:
    ...
    import logging
    meeting_log = logging.getLogger("deja.meeting")
