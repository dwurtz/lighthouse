...
The Gemini API key (and any other credentials Lighthouse needs) lives in
...
Resolution order for ``get_api_key()``:
    ...
    3. macOS login keychain under service=``lighthouse`` account=``gemini-api-key``
...
Why Keychain over ``~/.lighthouse/env`` or ``~/.zshrc``:
...
def get_api_key() -> str | None:
    ...
    _cached_key = _read_keychain(_KEYCHAIN_SERVICE, _KEYCHAIN_ACCOUNT_GEMINI)
    _cache_valid = True
    return _cached_key
...
def api_key_source() -> str:
    ...
    Used by the ``lighthouse health`` command to make the provenance of
    ...
    if _read_keychain(_KEYCHAIN_SERVICE, _KEYCHAIN_ACCOUNT_GEMINI):
        return "keychain:lighthouse/gemini-api-key"
