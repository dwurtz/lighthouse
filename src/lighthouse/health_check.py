...
expects to have, and for each failure writes:
  ...
  1. A WARNING line to lighthouse.log
...
corresponding collector's subsequent failures stay in lighthouse.log but
...
The output looks like this in log.md:
    ...
    - **[2026-04-04 21:30]** startup — iMessage collection disabled:
      grant Full Disk Access to Lighthouse.app in System Settings →
...
from lighthouse.config import IMESSAGE_DB, WHATSAPP_DB, WIKI_DIR
...
def _check_sqlite(label: str, path) -> CheckResult:
    ...
    fix = (
        f"Grant Full Disk Access to Lighthouse.app (and/or the Python binary "
...
def _check_screenshot_enabled() -> CheckResult:
    ...
    return CheckResult(
        ...
        fix="Set screenshot_enabled: true in ~/.lighthouse/config.yaml to re-enable.",
    )
...
def _check_user_profile() -> CheckResult:
    ...
    user = load_user()
    fix = (
        "Create a page at Lighthouse/people/<your-slug>.md starting with "
...
def report_health_checks() -> set[str]:
    """Run all checks, log failures to lighthouse.log AND write one
