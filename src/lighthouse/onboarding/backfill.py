...
Progress is tracked in ``~/.lighthouse/onboarding.json`` so steps only
...
MARKER_PATH = LIGHTHOUSE_HOME / "onboarding.json"
...
def _imessage_pre_check() -> dict[str, Any] | None:
    ...
    import sqlite3
    from lighthouse.config import IMESSAGE_DB
    ...
    try:
        ...
        return {
            ...
            "detail": str(e),
            "fix": (
                "Grant Full Disk Access to the Lighthouse Python binary "
...
async def backfill_imessage(
    *,
    ...
    from lighthouse.observations.imessage import fetch_imessage_contacts_backfill
    from lighthouse.onboarding.runner import run_step
...
def _whatsapp_pre_check() -> dict[str, Any] | None:
    ...
    import sqlite3
    from lighthouse.config import WHATSAPP_DB
...
async def backfill_whatsapp(
    *,
    ...
    from lighthouse.observations.whatsapp import fetch_whatsapp_contacts_backfill
    from lighthouse.onboarding.runner import run_step
...
async def run_all_pending_steps(
    *,
    ...
    ``lighthouse onboard --only imessage`` after granting Full Disk
